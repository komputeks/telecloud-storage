...
type Step = 'credentials' | 'profile' | 'complete';
          ...
          {step === 'credentials' && (
            <form onSubmit={handleCredentials} className="space-y-4">
              ...
              <button
                type="button"
                onClick={() => {
                  // Google sign-in via Supabase proxy would go here
                  // For now show info
          ...
          {step === 'profile' && payStep === 'idle' && (
            <form onSubmit={handleProfile} className="space-y-4">
              ...
              <div className="relative">
                ...
                <input type="tel" placeholder="M-Pesa phone (optional)" value={phone} onChange={(e) => setPhone(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 bg-[#1e1e2e] border border-[#27272a] rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-[#6366f1]" />
              </div>
              ...
              <div className="p-4 bg-gradient-to-br from-[#6366f1]/10 to-[#8b5cf6]/5 border border-[#6366f1]/20 rounded-xl">
                ...
                {!phone.trim() && (
                  <p className="text-xs text-gray-500 italic">Enter phone number above to enable payment</p>
                )}
              </div>
          ...
          {step === 'profile' && payStep === 'failed' && (
            <div className="text-center py-8">
              ...
              <div className="flex gap-3 justify-center">
                ...
                <button onClick={skipPayment} className="px-4 py-2 bg-[#6366f1] text-white rounded-xl text-sm">Continue Free</button>
              </div>
            </div>
          )}
        </div>
